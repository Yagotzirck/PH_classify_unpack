...
def paths_setup(cg_extractor: str) -> tuple[str, str, str]:
    if cg_extractor == '--radare2':
        return (
            paths.DISSMATS_RADARE2_PATH,
            paths.FIXED_THRESHOLDS_INTRACLUSTER_RADARE2_PATH,
            paths.CLUSTERING_RADARE2_PATH
        )
    elif cg_extractor == '--ghidra':
        return (
            paths.DISSMATS_GHIDRA_PATH,
            paths.FIXED_THRESHOLDS_INTRACLUSTER_GHIDRA_PATH,
            paths.CLUSTERING_GHIDRA_PATH
        )
    else:
        raise paths.UnspecifiedCallGraphGeneratorError()
    

def extract_clustering(cg_extractor: str):
    DISSIMILARITY_MATRICES_PATH, \
    FIXED_THRESHOLDS_INTRACLUSTER_PATH, \
    CLUSTERING_PATH = paths_setup(cg_extractor) 
    ...
    with open(FIXED_THRESHOLDS_INTRACLUSTER_PATH, 'wb') as f:
